...
def test_source_partitioning(temp_dir):
    ts = time.time()
    with RawArchive(temp_dir) as archive:
        archive.write(RawEvent(source='FEEDA', payload={}, receive_timestamp=ts, raw_id='1'))
        archive.write(RawEvent(source='FEEDB', payload={}, receive_timestamp=ts, raw_id='2'))
        archive.write(RawEvent(source='FEEDC', payload={}, receive_timestamp=ts, raw_id='3'))
...
def test_buffered_flush_on_close(temp_dir):
    events = [
        RawEvent(source='FEEDA', payload={}, receive_timestamp=time.time(), raw_id=str(i))
        for i in range(10)
    ]
...
def test_replay_filter_by_date(temp_dir):
    ...
    with RawArchive(temp_dir) as archive:
        archive.write(RawEvent(source='FEEDA', payload={}, receive_timestamp=ts1, raw_id='1'))
        archive.write(RawEvent(source='FEEDA', payload={}, receive_timestamp=ts2, raw_id='2'))
...
def test_stats(temp_dir):
    ...
    with RawArchive(temp_dir) as archive:
        archive.write(RawEvent(source='FEEDA', payload={}, receive_timestamp=ts, raw_id='1'))
        archive.write(RawEvent(source='FEEDB', payload={}, receive_timestamp=ts, raw_id='2'))
        archive.write(RawEvent(source='FEEDA', payload={}, receive_timestamp=ts, raw_id='3'))
