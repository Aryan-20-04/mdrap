...
class TestNewsPipeline(unittest.TestCase):
    ...
    def test_news_feed_add_and_query(self):
        self.feed.add_headline(
            "Apple beats earnings", "Source1", datetime(2026, 1, 1), "url1"
        )
        self.feed.add_headline(
            "Tesla misses earnings", "Source2", datetime(2026, 1, 2), "url2"
        )
        self.feed.add_headline(
            "Apple launches car", "Source3", datetime(2026, 1, 3), "url3"
        )
    ...
    def test_news_feed_sentiment_summary(self):
        self.feed.add_headline(
            "AAPL beats earnings expectations, raises guidance",
            "Source",
            datetime.now(),
            "url",
        )
        self.feed.add_headline(
            "AAPL faces lawsuit, shares plunge on recall warning",
            "Source",
            datetime.now(),
            "url",
        )
...
<item><title>Fed holds rates steady</title><link>https://example.com/2</link><pubDate>Mon, 01 Jan 2026 13:00:00 GMT</pubDate></item>
    ...
    def test_price_correlation(self):
        ...
        correlation = self.feed.correlate_with_price(
            item, price_before=150.0, price_after=155.0
        )
        self.assertIsNotNone(correlation)

    def test_urgency_detection(self):
        item = self.feed.add_headline(
            "BREAKING: Fed raises rates in emergency move", "Src", datetime.now(), "url"
        )
        if item is not None:
