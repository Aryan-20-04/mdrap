...
def test_cli_keys_and_audit(parser, tmp_path):
    ...
    _run_cmd(
        parser, ["keys", "create", "--client-id", "QuantFund_Alpha", "--rate", "1000"]
    )
...
def test_cli_live_and_research(parser, tmp_path):
    ...
    _run_cmd(
        parser, ["live", "ES.c.0", "-l", "2", "--mock-feed", "--feed", "databento"]
    )
...
def test_trading_cli_more_commands(parser):
    _run_cmd(
        parser,
        ["news", "analyze", "Nvidia beats earnings estimates revenue surges 50%"],
    )
