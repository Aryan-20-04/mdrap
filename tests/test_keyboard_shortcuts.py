...
from cli import KNOWN_SYMBOLS, MNEMONIC_MAP, QUICK_ACTIONS, ALL_CANONICAL_COMMANDS, build_parser
...
def test_single_ticker_render_hotkey_states():
    ...
    renderable = td.render_single_ticker('AAPL', paused=True, show_chart=False, show_depth=True)
...
def test_cli_ticker_first_and_shortcuts():
    ...
    test_cases = [
        ...
        (["cli.py", "p", "AAPL"], ["cli.py", "live", "AAPL", "--feed", "polygon", "--mock-feed"]),
        (["cli.py", "b", "ES"], ["cli.py", "live", "ES.c.0", "--feed", "databento", "--mock-feed"]),
    ...
    for argv_in, argv_expected in test_cases:
        with mock.patch("sys.argv", list(argv_in)):
            with mock.patch("argparse.ArgumentParser.parse_args") as mock_parse:
                ...
                main()
                assert sys.argv == argv_expected, f"Failed for {argv_in}: got {sys.argv} expected {argv_expected}"
