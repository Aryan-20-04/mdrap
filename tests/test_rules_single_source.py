...
def test_gen_reasons_check_cli():
    ...
    assert res.returncode == 0, f"gen_reasons.py --check failed:\n{res.stdout}\n{res.stderr}"
...
def test_native_c_never_sets_user_bits():
    ...
    for ev in test_events:
        res = _CFastResult()
        _NATIVE_LIB.fastpath_engine_evaluate(engine._engine_ptr, ctypes.byref(ev), ctypes.byref(res))
        assert (res.reason_mask & user_mask) == 0, f"Native C set user bits (>= 32): {hex(res.reason_mask)}"
