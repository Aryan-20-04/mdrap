...
def test_native_sbe_stream_validation_rules():
    ...
    raw_buf[0:128] = pack_sbe_tick(
        seq=101, symbol="AAPL", source="FEEDX", price=150.0, size=100.0,
        bid=149.9, ask=150.1, status="VALID"
    )
    # Tick 1: Crossed quote (bid 150.5 > ask 150.0)
    raw_buf[128:256] = pack_sbe_tick(
        seq=102, symbol="AAPL", source="FEEDX", price=150.0, size=100.0,
        bid=150.5, ask=150.0, status="VALID"
    )
    # Tick 2: Negative price (-10.0)
    raw_buf[256:384] = pack_sbe_tick(
        seq=103, symbol="AAPL", source="FEEDX", price=-10.0, size=100.0,
        bid=149.9, ask=150.1, status="VALID"
    )
