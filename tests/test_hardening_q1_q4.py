...
def test_q1_per_source_instrument_sequence_and_dedup():
    ...
    ev2 = _make_event(source="FEED_B", instrument="AAPL", seq=1)  # Lower sequence, but different source
...
def test_q2_clean_only_baseline():
    ...
    for seq in range(1, 21):
        ev = _make_event(source="FEED_A", instrument="BTC/USD", ex_ts=1000.0 + seq, rc_ts=1000.0 + seq + 0.001, seq=seq, price=100.0)
        eng.evaluate(ev)
    ...
    dup_ev = _make_event(source="FEED_A", instrument="BTC/USD", ex_ts=1020.0, rc_ts=1020.001, seq=20, price=999.0)
...
def test_q3_watermark_poisoning_guard():
    ...
    # Normal tick
    ev1 = _make_event(source="FEED_A", instrument="AAPL", ex_ts=1000.0, rc_ts=1000.01, seq=1, price=150.0)
    eng.evaluate(ev1)

    # Bogus future timestamp (exchange_ts = 2000.0 when receive_ts = 1000.02)
    ev_future = _make_event(source="FEED_A", instrument="AAPL", ex_ts=2000.0, rc_ts=1000.02, seq=2, price=150.0)
    ...
    ev3 = _make_event(source="FEED_A", instrument="AAPL", ex_ts=1000.03, rc_ts=1000.04, seq=3, price=150.0)
...
def test_q4_warmup_and_regime_shift():
    ...
    for seq in range(1, 10):
        ...
        ev = _make_event(source="FEED_A", instrument="AAPL", ex_ts=1000.0 + seq, rc_ts=1000.0 + seq + 0.001, seq=seq, price=px)
        res = eng.evaluate(ev)
    ...
    for seq in range(10, 26):
        ev = _make_event(source="FEED_A", instrument="AAPL", ex_ts=1000.0 + seq, rc_ts=1000.0 + seq + 0.001, seq=seq, price=100.0)
        eng.evaluate(ev)
    ...
    for i in range(1, 9):
        seq = 25 + i
        ev = _make_event(source="FEED_A", instrument="AAPL", ex_ts=1000.0 + seq, rc_ts=1000.0 + seq + 0.001, seq=seq, price=115.0)
        res = eng.evaluate(ev)
        if i < 8:
    ...
    ev_post = _make_event(source="FEED_A", instrument="AAPL", ex_ts=1000.0 + 34, rc_ts=1000.0 + 34 + 0.001, seq=34, price=115.0)
