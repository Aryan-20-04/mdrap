...
from chd import (CHDClient, CHDError, HistoricalRequest, IntegrityError,
                 MissingPartition, HistoricalRecord, Partition, received_ns)
...
def request(data_type='trades', start='2025-08-01T20:00:00Z', end='2025-08-01T21:00:00Z'):
    return HistoricalRequest('binance_spot', 'BTCUSDT', data_type, start, end)


def trade(offset=0, **updates):
    row = dict(received_time=BASE * 10**9 + offset, event_time=BASE * 1000,
               trade_time=BASE * 1000, trade_id=42, symbol='BTCUSDT',
               price='113328.68000000', quantity='0.00156000', is_buyer_maker=False,
               order_type='MARKET')
    row.update(updates)
    return row
...
def level(offset=0, kind='snapshot', side='bid', price='100', quantity='2', **updates):
    row = dict(received_time=BASE * 10**9 + offset, event_time=BASE * 1000 + offset // 10**6,
               symbol='BTCUSDT', event_type=kind, side=side, price=price, quantity=quantity,
               final_update_id=10, first_update_id=None, prev_final_update_id=None,
               last_update_id=None)
    row.update(updates)
    return row
...
def parquet(rows):
    buf = io.BytesIO()
    pq.write_table(pa.Table.from_pylist(rows), buf, compression='zstd', row_group_size=2)
    return buf.getvalue()
...
def http_error(code, headers=None):
    return HTTPError('https://api.cryptohftdata.com/v1/download', code, 'error', headers or {}, None)
...
@pytest.mark.parametrize('updates', [dict(start='2025-08-01T20:00:00'), dict(end='2025-08-01'),
    dict(exchange='binance'), dict(symbol='../BTC'), dict(symbol='BTC/USD'),
    dict(data_type='mark_price'), dict(symbol=''), dict(symbol='a\nb')])
def test_invalid_request(updates):
    values = dict(exchange='binance_spot', symbol='BTCUSDT', data_type='trades', start='2025-08-01', end='2025-08-02')
...
def test_exact_nanosecond_interval_and_stable_identity(tmp_path, network):
    c = client_with_rows(tmp_path, network, [trade(999), trade(1000), trade(1999), trade(2000)])
...
def test_jwt_header_refresh_and_no_credential_leak(tmp_path, network):
    calls, replies = network
    replies.extend([Response(b'{"jwt_token":"first"}'), http_error(401),
                    Response(b'{"jwt_token":"second"}'), Response(parquet([trade()]))])
...
def test_retries_rate_limit_server_and_network(tmp_path, network):
    calls, replies = network
    replies.extend([http_error(429, {'Retry-After': '2'}), http_error(503),
                    URLError('secret from remote'), Response(parquet([trade()]))])
...
@pytest.mark.parametrize('body', [b'', b'<html>error</html>', b'PAR1brokenPAR1', b'\x28\xb5\x2f\xfdbroken'])
...
def test_truncation_retried(tmp_path, network):
    calls, replies = network
    body = parquet([trade()])
    replies.extend([Response(body, {'Content-Length': str(len(body) + 1)}), Response(body)])
    CHDClient(cache_dir=tmp_path).fetch(next(request().partitions()))
    assert len(calls) == 2
...
def test_symbol_discovery_and_schema_validation(tmp_path, network):
    calls, replies = network
    replies.extend([Response(b'{"symbols":["BTCUSDT","DELISTED"]}'), Response(b'{"symbols":null}')])
    c = CHDClient(cache_dir=tmp_path)
...
@pytest.mark.parametrize('updates', [dict(price='NaN'), dict(quantity='0'), dict(price='Infinity'),
                                     dict(price='1e-1000'), dict(symbol='ETHUSDT')])
...
def book_rows():
    return [level(), level(side='ask', price='102'),
            level(10**9, 'update', price='101', final_update_id=11, prev_final_update_id=10),
            level(2 * 10**9, 'update', price='101', quantity='0', final_update_id=12, prev_final_update_id=11),
...
def test_orderbook_atomic_snapshot_update_delete_and_reset(tmp_path, network):
    ...
    assert [(e.payload['bid'], e.payload['ask']) for e in events] == [(100, 102), (101, 102), (100, 102), (90, 91)]
...
@pytest.mark.parametrize('rows', [[level(kind='update')],
    [level(), level(side='ask', price='102'), level(10**9, 'update', prev_final_update_id=9, final_update_id=11)],
    [level(), level(side='ask', price='102'), level(10**9, 'update', first_update_id=15, final_update_id=16)],
    [level(side='buy')]])
...
def test_pipeline_import_raw_replay_lineage_and_no_false_age(tmp_path, network):
    ...
    rows = original.conn.execute('SELECT source, reasons FROM canonical_events').fetchall()
...
def test_cli_plan_alias_json_and_validation(capsys):
    ...
    p = build_parser()
    args = p.parse_args(['chd', 'plan', '--exchange', 'binance_spot', '--symbol', 'BTCUSDT',
                         '--start', '2025-08-01', '--end', '2025-08-02', '--json'])
    args.func(args)
...
def test_cli_download_partial_json_exit_two(tmp_path, network, capsys):
    ...
    args = build_parser().parse_args(['historical', 'download', '--exchange', 'binance_spot',
        '--symbol', 'BTCUSDT', '--start', '2025-08-01T20:00:00Z', '--end', '2025-08-01T21:00:00Z',
        '--cache-dir', str(tmp_path), '--allow-missing', '--json'])
    with pytest.raises(SystemExit) as err:
        args.func(args)
...
def test_warmup_skips_unknown_preinterval_state_only(tmp_path, network):
    rows = [level(kind='update', final_update_id=1),
            level(10**9), level(10**9, side='ask', price='102'),
...
def test_cross_hour_book_continuity(tmp_path, network):
    calls, replies = network
    replies.extend([Response(parquet([level(), level(side='ask', price='102')])),
                    Response(parquet([level(3600 * 10**9, 'update', price='101',
                                            final_update_id=11, prev_final_update_id=10)]))])
    c = CHDClient(cache_dir=tmp_path / 'cache')
...
@pytest.mark.parametrize('data_type', ['ticker', 'mark_price', 'open_interest', 'liquidations'])
def test_native_research_datasets_preserve_nulls_and_extra_fields(tmp_path, network, data_type):
    calls, replies = network
    row = dict(received_time=BASE * 10**9, symbol='BTCUSDT', funding_rate='0.0000123456789',
               index_price=None, timestamp=BASE * 1000, venue_extension='preserve me')
    replies.append(Response(parquet([row, row])))
    c = CHDClient(cache_dir=tmp_path)
    r = HistoricalRequest('binance_futures', 'BTCUSDT', data_type, '2025-08-01T20:00:00Z', '2025-08-01T21:00:00Z')
