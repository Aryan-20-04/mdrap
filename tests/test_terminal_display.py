...
def test_render_candlestick_chart():
    ...
    chart = render_candlestick_chart(
        candles, width=40, height=8, show_volume=True, title="Test Chart"
    )
...
def test_live_ticker_dashboard_single_ticker():
    ...
    ev_quote = CanonicalEvent(
        ...
        exchange_timestamp=1000.002,
        receive_timestamp=1000.003,
        processing_timestamp=1000.004,
        source="COINBASE",
        sequence_number=2,
        bid_price=64995.0,
        ask_price=65005.0,
...
def test_cli_ticker_command(temp_env):
    ...
    args = parser.parse_args(
        ["ticker", "BTC/USD", "--sim", "--limit", "4", "--db", temp_env["db"]]
    )
    assert args.func is not None
