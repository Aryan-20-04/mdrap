...
def _make_events(instrument='AAPL', n=100, start_price=100.0, end_price=110.0, start_ts=1000.0):
    ...
    for i in range(n):
        ...
        events.append(CanonicalEvent(
            event_id=f'evt-{i}', instrument_id=instrument, event_type=EventType.TRADE,
            exchange_timestamp=t, receive_timestamp=t, processing_timestamp=t,
            source='TEST', sequence_number=i, price=price, quantity=100.0,
...
class TestBacktestEngine(unittest.TestCase):
    ...
    def test_backtest_metrics_correctness(self):
        ...
        for i in range(100):
            tick_idx = i + 1
            price = 100.0
            if tick_idx == 10: price = 100.0
            elif tick_idx == 50: price = 110.0
            elif tick_idx == 60: price = 110.0
            elif tick_idx == 90: price = 100.0
            
            events.append(CanonicalEvent(
                event_id=f'evt-{i}', instrument_id='AAPL', event_type=EventType.TRADE,
                exchange_timestamp=1000.0+i, receive_timestamp=1000.0+i, processing_timestamp=1000.0+i,
                source='TEST', sequence_number=i, price=price, quantity=100.0,
                quality_status=QualityStatus.VALID
            ))
    ...
    def test_backtest_max_drawdown(self):
        ...
        for i, p in enumerate(prices):
            events.append(CanonicalEvent(
                event_id=f'evt-{i}', instrument_id='AAPL', event_type=EventType.TRADE,
                exchange_timestamp=1000.0+i, receive_timestamp=1000.0+i, processing_timestamp=1000.0+i,
                source='TEST', sequence_number=i, price=p, quantity=100.0,
                quality_status=QualityStatus.VALID
            ))
    ...
    def test_walkforward_splits(self):
        ...
        results = engine.run_walkforward(strategy_factory, events, n_splits=3, train_pct=0.6)
