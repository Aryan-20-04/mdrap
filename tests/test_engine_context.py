...
def test_multi_engine_dedup_and_sequence_isolation():
    cfg = QualityConfig(staleness_threshold_s=10.0, price_anomaly_stddev=4.0, price_window=50)
    ...
    try:
        ...
        ev1_seq2 = make_event(sequence_number=2, exchange_timestamp=1000.1, receive_timestamp=1000.101, price=150.1)
        assert eng1.evaluate(ev1_seq2).quality_status == QualityStatus.VALID

        ev2_seq5 = make_event(sequence_number=5, exchange_timestamp=1000.2, receive_timestamp=1000.201, price=150.2)
        ...
        ev1_seq3 = make_event(sequence_number=3, exchange_timestamp=1000.3, receive_timestamp=1000.301, price=150.15)
...
def test_multi_engine_price_stats_isolation():
    cfg = QualityConfig(staleness_threshold_s=10.0, price_anomaly_stddev=3.0, price_window=30)
    eng_low = FastQualityEngine(cfg)
    eng_high = FastQualityEngine(cfg)

    try:
        for i in range(25):
            eng_low.evaluate(make_event(
                instrument_id="MSFT",
        ...
        res_low = eng_low.evaluate(make_event(
            instrument_id="MSFT",
        ...
        assert res_low.reasons == [], f"Unexpected reasons on res_low: {res_low.reasons}"
        assert res_low.quality_status == QualityStatus.VALID

        res_high = eng_high.evaluate(make_event(
            instrument_id="MSFT",
