...
def test_websocket_auth_via_query_param(ws_env):
    client = ws_env["client"]
    token = ws_env["token"]

    with client.websocket_connect(f"/v1/events/stream?token={token}") as ws:
        # Handshake ACK is delivered upon connecting
        ack = ws.receive_json()
        assert ack["type"] == "ACK"
...
def test_websocket_rejects_unauthenticated(ws_env):
    ...
    with client.websocket_connect("/v1/events/stream?token=invalid_token_123") as ws:
        err = ws.receive_json()
