...
from options import (
    OptionType, ExerciseStyle, OptionContract, Greeks, OptionPrice,
    bsm_price, bsm_greeks, price_option, binomial_price, implied_volatility,
    put_call_parity_check, VolatilitySurface, OptionsChain
)

def test_bsm_call_known_value():
    """BSM call: S=100, K=100, T=1yr, r=5%, sigma=20%. Known theoretical ~10.45. Verify within 0.5."""
    price = bsm_price(S=100.0, K=100.0, T=1.0, r=0.05, sigma=0.20, option_type=OptionType.CALL)
    assert abs(price - 10.45) < 0.5

def test_bsm_put_known_value():
    """BSM put: same params. Known theoretical ~5.57. Verify within 0.5."""
    price = bsm_price(S=100.0, K=100.0, T=1.0, r=0.05, sigma=0.20, option_type=OptionType.PUT)
