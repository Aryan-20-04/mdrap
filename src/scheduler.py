...
class ScheduledJob:
    job_id: str
    name: str
    cron_expr: str            # e.g. '0 16 * * 1-5' (minute hour dom month dow) or '@daily', '@hourly'
...
class CronParser:
    ...
    def next_run_after(cls, cron_expr: str, after_timestamp: float) -> float:
        ...
        dows = cls.parse_field(parts[4], 0, 6) # 0 is Sunday, 1 is Monday ... 6 is Saturday
        ...
        for _ in range(5 * 365 * 24 * 60):
            if (dt.month in months and
                ...
                dt.isoweekday() % 7 in dows): # isoweekday: Mon=1...Sun=7 -> Mon=1...Sun=0
                return dt.timestamp()
...
class Scheduler:
    ...
    def add_job(self, name: str, cron_expr: str, action: Callable[[], Any], job_id: str | None = None) -> ScheduledJob:
        if job_id is None:
    ...
    def generate_eod_report(self, portfolio_tracker: Any = None, bar_db: Any = None) -> dict:
        ...
        if bar_db:
            try:
                ...
                report['market_data']['total_bars'] = getattr(bar_db, 'total_bar_count', 0)
            except Exception:
                pass
