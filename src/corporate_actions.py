...
class SymbolHistoryList(list):
    def __contains__(self, item: Any) -> bool:
        if super().__contains__(item):
            return True
        return any(item in (d.get('old_symbol'), d.get('new_symbol')) for d in self if isinstance(d, dict))
...
class CorporateActionsEngine:
    ...
    def _load_from_db(self) -> None:
        ...
        cursor.execute('SELECT action_id, action_type, symbol, effective_date, effective_timestamp, ratio, amount, new_symbol, description FROM corporate_actions ORDER BY action_id')
    ...
    def add_split(self, symbol: str, effective_date: str, ratio: float, description: str = '') -> CorporateAction:
    ...
    def add_dividend(self, symbol: str, effective_date: str, amount: float, description: str = '') -> CorporateAction:
    ...
    def add_ticker_change(self, old_symbol: str, new_symbol: str, effective_date: str, description: str = '') -> CorporateAction:
    ...
    def add_delisting(self, symbol: str, effective_date: str, description: str = '') -> CorporateAction:
        """Record a delisting."""
        action = CorporateAction(
            action_id=next(self._action_id_gen),
    ...
    def add_spinoff(self, parent_symbol: str, child_symbol: str, effective_date: str, ratio: float, description: str = '') -> CorporateAction:
    ...
    def adjust_price(self, symbol: str, raw_price: float, as_of_date: str, adjustment: str = 'split') -> float:
        ...
        if adjustment == 'split':
            ...
            total_divs = sum(a.amount for a in self.actions_for(symbol) if a.action_type == ActionType.CASH_DIVIDEND and a.effective_date > as_of_date)
    ...
    def resolve_symbol(self, symbol: str) -> str:
        ...
        for action in self._actions:
            if action.action_type == ActionType.TICKER_CHANGE and action.symbol == current:
    ...
    def symbol_history(self, current_symbol: str) -> list[dict]:
        ...
        while found:
            ...
            for action in reversed(self._actions):
                if action.action_type == ActionType.TICKER_CHANGE and action.new_symbol == target:
                    history.append({
    ...
    def is_active(self, symbol: str) -> bool:
        ...
        for action in self.actions_for(symbol):
            if action.action_type == ActionType.DELISTING:
                return False
            if action.action_type == ActionType.TICKER_CHANGE and action.symbol == symbol:
    ...
    def actions_for(self, symbol: str) -> list[CorporateAction]:
        ...
        symbol_actions = [a for a in self._actions if a.symbol == symbol or a.new_symbol == symbol]
...
WELL_KNOWN_ACTIONS = [
    ...
    ('FB', 'TICKER_CHANGE', '2021-10-28', 0, 'Facebook rebranded to Meta Platforms', 'META'),
...
def load_well_known(engine: CorporateActionsEngine) -> None:
    for action in WELL_KNOWN_ACTIONS:
        symbol, type_str, eff_date, ratio_or_amount, desc = action[0], action[1], action[2], action[3], action[4]
