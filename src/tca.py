...
class TCAMetrics:
    ...
    slippage_cents: (
        float  # Positive = unfavorable slippage, Negative = price improvement
    )
    slippage_bps: float
...
class BrokerScorecard:
    ...
    def price_improvement_rate_pct(self) -> float:
        return (
            round((self.price_improved_count / self.order_count) * 100.0, 1)
...
class TCAEngine:
    ...
    def evaluate_execution(
        ...
        prevailing_bbo: ConsolidatedBBO | None = None,
    ) -> TCAMetrics:
        """
        ...
        px = (
            record.price
            if (math.isfinite(record.price) and record.price > 0)
            else 100.0
        )
        shs = (
            record.shares
            if (math.isfinite(record.shares) and record.shares > 0)
            else 0.0
        )
        ...
        if (
            bbo is None
            or bbo.best_bid is None
            or bbo.best_ask is None
            or not math.isfinite(bbo.best_bid)
            or not math.isfinite(bbo.best_ask)
        ):
            bid = round(px - 0.05, 4)
            ask = round(px + 0.05, 4)
        else:
        ...
        arrival = (
            ...
            if (
                record.arrival_price is not None
                and math.isfinite(record.arrival_price)
                and record.arrival_price > 0
            )
            else mid
        )
        ...
        # 1. Slippage calculation (positive = unfavorable slippage, negative = favorable price improvement)
        if side_norm == "BUY":
        ...
        effective_spread_bps = (
            (effective_spread_cents / mid) * 10_000.0 if mid > 0 else 0.0
        )
        ...
        es_ratio = (
            effective_spread_cents / quoted_spread_cents
            if quoted_spread_cents > 0
            else 1.0
        )
        if is_price_improved:
            quality_score = min(
                100.0, 85.0 + (price_improvement_cents / quoted_spread_cents) * 15.0
            )
        elif is_disimproved:
    ...
    def evaluate_batch(self, records: list[ExecutionRecord]) -> dict[str, Any]:
        ...
        mean_eff_spread_bps = (
            sum(m.effective_spread_bps for m in metrics) / total_trades
        )
        mean_quoted_spread_bps = (
            sum(m.quoted_spread_bps for m in metrics) / total_trades
        )
        ...
        total_slippage_cost_usd = sum(
            max(0.0, m.slippage_cents) * m.shares for m in metrics
        )
        ...
        for broker, b_metrics in broker_groups.items():
            ...
            b_tot_slip_usd = sum(
                max(0.0, m.slippage_cents) * m.shares for m in b_metrics
            )
        ...
        compliance_status = (
            "COMPLIANT (PASSED BEST EXECUTION)"
            if overall_quality_score >= 65.0
            else "NON-COMPLIANT (FLAGGED FOR SLIPPAGE AUDIT)"
        )
        ...
        return {
            ...
            "price_improvement_rate_pct": round(
                (improved_count / total_trades) * 100.0, 1
            ),
            "price_disimproved_count": disimproved_count,
            "price_disimproved_rate_pct": round(
                (disimproved_count / total_trades) * 100.0, 1
            ),
    ...
    def generate_demo_executions(
        symbol: str = "AAPL", count: int = 100, seed: int | None = None
        ...
        - Broker 4 (Institutional Dark/Block): Controlled mid-market executions

        Args:
            symbol: Ticker symbol (e.g. 'AAPL', 'MSFT', 'BTC/USD').
            count: Number of synthetic execution records to generate.
            seed: Optional integer seed for reproducible drift and noise.

        Returns:
            list[ExecutionRecord]: Chronologically sorted execution records.
        """
