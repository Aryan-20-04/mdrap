...
class FIXMessage:
    ...
    def encode(self, sender: str = 'MDRAP', target: str = 'BROKER', seq_num: int = 1) -> str:
        ...
        msg_type = self.fields.get(FIXTag.MSG_TYPE, '')
        ...
        for tag, value in sorted(self.fields.items()):
            if tag not in header_tags and tag not in trailer_tags and tag not in ordered_body_tags:
        ...
        if fastpath is not None:
            ...
            checksum = c_sum if c_sum is not None else sum(ord(c) for c in msg_without_checksum) % 256
        else:
...
class FIXSession:
    ...
    def __init__(self, sender_comp_id: str, target_comp_id: str, fix_version: str = 'FIX.4.2'):
    ...
    def _prepare_and_encode(self, msg: FIXMessage) -> FIXMessage:
        ...
        # We don't encode here, we just increment seq for external encoding if needed
        # Actually, the caller probably expects the FIXMessage object returned directly, ready to be encoded
        seq = self.out_seq
        self.out_seq += 1
        return msg
    ...
    def create_new_order_single(self, cl_ord_id: str, symbol: str, side: str, qty: float, ord_type: str = '1', price: float | None = None) -> FIXMessage:
        msg = FIXMessage('D')
        ...
        msg.set(FIXTag.TRANSACT_TIME, datetime.now(timezone.utc).strftime('%Y%m%d-%H:%M:%S.%f')[:21])
        return self._prepare_and_encode(msg)

    def create_order_cancel(self, cl_ord_id: str, orig_cl_ord_id: str, symbol: str, side: str) -> FIXMessage:
        msg = FIXMessage('F')
        msg.set(FIXTag.CL_ORD_ID, cl_ord_id)
