...
class SourceWatchdog:
    def __init__(self, reliability: ReliabilityTracker,
        ...
        for source, state in self._source_states.items():
            ...
            if new_state != state:
                ...
                if new_state == SourceState.SILENT:
                    alert_type = 'SILENCE'
                    details = f"Source {source} silent for > {self.silence_threshold_s}s"
    ...
    def active_sources(self) -> list[str]:
        ...
        return [src for src, state in self._source_states.items() if state == SourceState.HEALTHY]
