...
def timestamp_ns(value, unit='auto') -> int:
    ...
    candidates = [value * scale for name, scale in scales.items()
                  if unit in ('auto', name) and 946684800 * 10**9 <= value * scale < 4102444800 * 10**9]
    if len(candidates) != 1:
        raise IntegrityError('Cannot resolve CHD timestamp unit; specify timestamp_unit explicitly')
    return candidates[0]
...
def _number(value, field, *, zero=False) -> Decimal:
    try:
        ...
        raise IntegrityError(f'CHD {field} must be a finite {"nonnegative" if zero else "positive"} number') from None


def _raw(record, sequence, event_type, exchange_ns, fields, *, first=None):
    p = record.partition
    payload = dict(instrument=f'CHD:{p.exchange}:{p.symbol}', event_type=event_type,
                   exchange_ts=exchange_ns / 10**9, sequence=sequence, **fields)
    payload['chd'] = dict(provider='chd', exchange=p.exchange, symbol=p.symbol,
    ...
    return RawEvent(source=f'CHD_{p.exchange}_{p.data_type}', payload=payload,
                    receive_timestamp=received_ns(record.values) / 10**9, raw_id=record.raw_id)


def _check_symbol(record):
    if record.values.get('symbol') != record.partition.symbol:
        raise IntegrityError(f'CHD row symbol does not match requested market: {record.partition.key}, row {record.row_index}')
...
class OrderBook:
    ...
    def message_key(record):
        row = record.values
        return tuple(row.get(k) for k in ('received_time', 'event_time', 'event_type',
    ...
    def apply(self, records):
        ...
        for key in ('first_update_id', 'final_update_id', 'prev_final_update_id', 'last_update_id'):
            val = row.get(key)
        ...
        if kind == 'snapshot':
            ...
            raise IntegrityError('Orderbook starts with updates; request an earlier interval containing a snapshot')
            ...
            if previous is not None and previous != self.last_update_id:
                raise IntegrityError('CHD orderbook sequence gap (prev_final_update_id)')
        ...
        return first, last, dict(bid=float(bid), ask=float(ask),
                                bid_size=float(self.bids[bid]), ask_size=float(self.asks[ask]))


def iter_events(client: CHDClient, request: HistoricalRequest, *, timestamp_unit='auto', book_start=None):
    ...
    if request.data_type not in ('trades', 'orderbook'):
        raise ValueError('Canonical ingestion supports trades and orderbook; use iter_records/download for other datasets')
    ...
    records = client.iter_records(read_request, include_warmup=request.data_type == 'orderbook')
    if request.data_type == 'trades':
        ...
        for record in records:
            ...
            if isinstance(trade_id, bool) or not isinstance(trade_id, (int, str)) or trade_id == '':
                raise IntegrityError('CHD trade_id must be an integer or nonempty string')
            ...
            fields = dict(price=float(_number(row.get('price'), 'price')),
        ...
        book = OrderBook()
        start_ns = int(request.start.timestamp()) * 10**9 + request.start.microsecond * 1000
        sequence = 0
        for _, group in itertools.groupby(records, key=book.message_key):
            group = iter(group)
            head = next(group)
            if not book.initialized and head.values.get('event_type') == 'update' and received_ns(head.values) < start_ns:
            ...
            fallback = value in (None, 0) and last.values.get('event_type') == 'snapshot'
            exchange_ns = received_ns(last.values) if fallback else timestamp_ns(value, timestamp_unit)
            raw = _raw(last, sequence, 'QUOTE', exchange_ns, quote, first=first)
            raw.payload['chd']['timestamp_basis'] = 'received_time' if fallback else 'event_time'
            yield raw
...
def _read_request(request, book_start):
    ...
    return HistoricalRequest(request.exchange, request.symbol, request.data_type, start, request.end)


def ingest_history(client: CHDClient, request: HistoricalRequest, output: str | Path,
                   *, timestamp_unit='auto', book_start=None) -> dict:
    ...
    if request.data_type not in ('trades', 'orderbook'):
        raise ValueError('Ingest supports trades/orderbook; use download for native datasets')
    output = Path(output)
    if output.exists():
        raise FileExistsError(f'Historical output already exists: {output}; choose a new run directory')
    ...
    manifest['book_start'] = utc_datetime(book_start).isoformat() if book_start is not None else None
    ...
    try:
        ...
        try:
            with RawArchive(str(temporary / 'raw')) as archive:
                ...
                count = 0
                for raw in iter_events(client, request, timestamp_unit=timestamp_unit, book_start=book_start):
                    pipeline.process_one(raw)
                    count += 1
                pipeline.finish()
                if not count:
                    raise CHDError('No canonical events in requested interval; no run was published')
