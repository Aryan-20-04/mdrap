...
def validate_url_security(url: str) -> None:
    ...
    if not host or host.lower() not in ALLOWED_SEC_HOSTS:
        raise SecurityError(f'Host not permitted: {host!r}. SEC requests restricted to official SEC hosts.')
...
def validate_xml_security(xml_bytes: bytes, max_bytes: int = MAX_XML_BYTES) -> bytes:
    ...
    if len(xml_bytes) > max_bytes:
        raise SecurityError(f'XML payload exceeds size ceiling of {max_bytes} bytes (got {len(xml_bytes)})')

    upper = xml_bytes.upper()
    if b'<!ENTITY' in upper or b'SYSTEM ' in upper or b'PUBLIC ' in upper:
        raise SecurityError('Malicious XML detected: entity declarations and external entities are strictly prohibited')
...
ITEM_TAXONOMY: Dict[str, Tuple[str, str, str]] = {
    ...
    "2.02": ("Results of Operations & Financial Condition (Earnings)", "Financials", "📊"),
    "2.03": ("Creation of Direct Financial Obligation / Debt", "Debt & Financing", "💳"),
    "2.04": ("Acceleration of Direct Financial Obligation / Default", "Credit Risk", "🚨"),
    "2.05": ("Costs Associated with Exit / Restructuring / Layoffs", "Restructuring", "✂️"),
    "2.06": ("Material Impairments / Asset Write-Down", "Asset Write-down", "📉"),
    "3.01": ("Notice of Delisting / Non-Compliance with Listing Rules", "Listing Risk", "🚫"),
    "3.02": ("Unregistered Sales of Equity Securities", "Dilution", "💵"),
    "3.03": ("Material Modification to Rights of Security Holders", "Shareholder Rights", "⚖️"),
    "4.01": ("Changes in Registrant's Certifying Accountant", "Auditor Change", "🔍"),
    "4.02": ("Non-Reliance on Financials / Accounting Restatement", "Restatement Risk", "🚨"),
    "5.01": ("Changes in Control of Registrant", "Takeover / Control", "👑"),
    "5.02": ("Departure / Election of Directors or Principal Officers", "Executive Leadership", "👤"),
    "5.03": ("Amendments to Articles of Incorporation or Bylaws", "Corporate Governance", "📜"),
    "5.04": ("Temporary Suspension of Trading Under Benefit Plans", "Trading Lockup", "🔒"),
    "5.05": ("Amendments / Waiver to Code of Ethics", "Ethics", "⚖️"),
    "5.06": ("Change in Shell Company Status", "Corporate Structure", "🐚"),
    "5.07": ("Submission of Matters to a Vote of Security Holders", "Shareholder Vote", "🗳️"),
...
def parse_form4_xml(
    ...
    for tx in tx_nodes:
        ...
        acq_disp = (tx.findtext('.//transactionAcquiredDisposedCode/value') or '').strip().upper()
        ...
        direct = (tx.findtext('.//directOrIndirectOwnership/value') or 'D').strip().upper()
...
class EdgarClient:
    def __init__(
        ...
        self._submissions_mem_cache: Dict[str, Tuple[dict, float]] = {}
        self._facts_mem_cache: Dict[str, Tuple[dict, float]] = {}
    ...
    def _get_cached_sec_json(self, subfolder: str, cik: str, url_template: str, fresh: bool = False) -> dict:
        cik_clean = str(int(cik)).zfill(10)
        prefix = 'sub' if subfolder == 'submissions' else 'facts'
        cache_key = f"{prefix}_{cik_clean}"
        mem_cache = self._submissions_mem_cache if subfolder == 'submissions' else self._facts_mem_cache
        now = time.time()
    ...
    def _get_submissions(self, cik: str, fresh: bool = False) -> dict:
        ...
        return self._get_cached_sec_json('submissions', cik, SEC_SUBMISSIONS_URL, fresh=fresh)
    ...
    def get_material_events(self, ticker: str, limit: int = 15, fresh: bool = False) -> List[MaterialEvent]:
        clean_ticker = sanitize_ticker(ticker)
        cik = self.lookup_cik(clean_ticker)
        data = self._get_submissions(cik, fresh=fresh)
    ...
    def get_insiders(self, ticker: str, limit: int = 20, fresh: bool = False) -> List[FilingRecord]:
        clean_ticker = sanitize_ticker(ticker)
        return self.get_filings(clean_ticker, form_type='4', limit=limit, fresh=fresh)

    def get_insider_trades(self, ticker: str, limit: int = 10, fresh: bool = False) -> List[InsiderTrade]:
