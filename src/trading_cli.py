...
def cmd_backtest(args: argparse.Namespace) -> None:
    ...
    strategy = (
        WhaleMomentumStrategy(symbol=symbol)
        if strat_name == "whale_momentum"
        else SpreadCaptureMarketMaker(symbol=symbol)
    )
    ...
    console.print(
        _t(
            ...
            [("Metric", {"style": "cyan"}), ("Value", {"style": "bold green"})],
            [
                ...
                [
                    ...
                    if not math.isinf(res.sortino_ratio)
                    else "N/A",
                ],
                [
                    "Calmar Ratio",
                    f"{res.calmar_ratio:.2f}"
                    if not math.isinf(res.calmar_ratio)
                    else "N/A",
                ],
                ...
                [
                    ...
                    if not math.isinf(res.profit_factor)
                    else "N/A",
                ],
            ],
            border="bright_blue",
...
def cmd_risk(args: argparse.Namespace) -> None:
    ...
    console.print(
        _t(
            "Risk & VaR Analytics",
            [("Metric", {"style": "cyan"}), ("Value", {"style": "bold yellow"})],
            [
    ...
    with BarDatabase(db_path) as db:
        if action == "summary":
            ...
            console.print(
                _t(
                    None,
                    [("Metric", {"style": "cyan"}), ("Value", {"style": "green"})],
                    [
            ...
            bars = db.query_bars(
                symbol, interval=interval, limit=getattr(args, "limit", 15)
            )
            if not bars:
                console.print(
                    f"[yellow]No {interval} bars found for {symbol} in {db_path}.[/yellow]"
                )
                return
            console.print(
                _t(
                    f"Recent {interval} Bars: {symbol}",
                    [
                        ("Timestamp", {"style": "dim"}),
                        ("Open", {"justify": "right"}),
                        ("High", {"justify": "right", "style": "green"}),
                        ("Low", {"justify": "right", "style": "red"}),
                        ("Close", {"justify": "right"}),
                        ("Volume", {"justify": "right"}),
                        ("VWAP", {"justify": "right", "style": "yellow"}),
                    ],
                    [
                        [
                            time.strftime(
                                "%Y-%m-%d %H:%M:%S", time.gmtime(b.bucket_start)
                            ),
                            f"{b.open:.2f}",
                            f"{b.high:.2f}",
                            f"{b.low:.2f}",
...
def cmd_options(args: argparse.Namespace) -> None:
    ...
    opt_type = (
        OptionType.PUT
        if getattr(args, "type", "call").lower() == "put"
    ...
    if action == "price":
        ...
        console.print(
            _t(
                None,
                [("Property", {"style": "cyan"}), ("Value", {"style": "bold green"})],
                [
                    ["Theoretical Price", f"${res.theoretical:.4f}"],
                    ["Intrinsic Value", f"${res.intrinsic:.4f}"],
                    ["Time Value", f"${res.time_value:.4f}"],
                    ["Delta (dV/dS)", f"{res.greeks.delta:+.4f}"],
                    ["Gamma (d²V/dS²)", f"{res.greeks.gamma:+.4f}"],
                    ["Theta (per day)", f"${res.greeks.theta:+.4f}"],
                    ["Vega (per 1% vol)", f"${res.greeks.vega:+.4f}"],
                    ["Rho (per 1% rate)", f"${res.greeks.rho:+.4f}"],
                    ["Vanna", f"{res.greeks.vanna:+.4f}"],
                    ["Volga", f"{res.greeks.volga:+.4f}"],
                ],
                border="magenta",
            )
        )
        ...
        console.print(
            _t(
                f"Options Chain (Expiry: {expiry}d, Spot: ${spot:.2f})",
                [
                    ("Type", {"style": "bold"}),
                    ("Strike", {"justify": "right"}),
                    ("Price", {"justify": "right", "style": "green"}),
                    ("Delta", {"justify": "right"}),
                    ("Gamma", {"justify": "right"}),
                    ("Vega", {"justify": "right"}),
                    ("Theta", {"justify": "right"}),
...
def cmd_news(args: argparse.Namespace) -> None:
    ...
    from news import FinancialSentimentAnalyzer, TickerExtractor, NewsFeed
            ...
            try:
                ...
                for ev in sec_events:
                    desc = getattr(ev, "description", "") or (
                        ", ".join(getattr(ev, "decoded_items", [])) or "SEC 8-K Event"
                    )
                    feed.add_headline(
                        f"{target_sym}: {desc}",
                        source="SEC_8K",
                        url=getattr(ev, "filing_url", ""),
                    )
                if sec_events:
                    fetched = True
            except Exception as exc:
    ...
    if action in ("latest", "fetch"):
        ...
        for it in items:
            color = (
                "green"
                if "BULLISH" in it.sentiment.value
                else ("red" if "BEARISH" in it.sentiment.value else "yellow")
            )
            ...
            syms_str = (
                ", ".join(it.symbols)
                if it.symbols
                else (symbol.upper() if symbol else "-")
            )
            urgency_str = (
                f"[bold red]{it.urgency}[/bold red]"
                if it.urgency in ("HIGH", "CRITICAL")
                else it.urgency
            )
            rows.append(
                [
        ...
        cons_color = (
            ...
            else ("red" if consensus == "BEARISH" else "yellow")
        )
        console.print(
            _t(
                None,
                [("Metric", {"style": "cyan"}), ("Value", {"style": "bold"})],
                [
        ...
        text = (
            getattr(args, "text", "")
            or "Apple beats Q4 revenue expectations, raises dividend and buyback program"
        )
        ...
        color = (
            "green"
            if "BULLISH" in sentiment.value
            else ("red" if "BEARISH" in sentiment.value else "yellow")
        )
        console.print(
            _t(
                None,
                [("Field", {"style": "cyan"}), ("Value", {"style": "bold"})],
                [
...
def cmd_alert(args: argparse.Namespace) -> None:
    ...
    if action == "add":
        symbol, atype, target = (
            getattr(args, "symbol", "AAPL"),
            getattr(args, "type", "ABOVE"),
            getattr(args, "target", 200.0),
        )
        a = engine.add_price_alert(symbol, atype, target)
        console.print(
            f"[bold green]Alert created:[/bold green] ID={a.alert_id} for {symbol} {atype} {target}"
        )
    elif action == "list":
        console.print(
            _t(
                "Configured Trading Alerts",
                [
                    ("ID", {"style": "dim"}),
    ...
    if action == "list":
        console.print(
            _t(
                ...
                [[w.name, ", ".join(w.symbols), w.description] for w in wm.list_all()],
                border="green",
            )
        )
    elif action == "add":
...
def cmd_portfolio(args: argparse.Namespace) -> None:
    ...
    tracker = PortfolioTracker(
        ...
        db_path=getattr(args, "db", "data/portfolio.db"),
    )
    if (getattr(args, "action", "summary") or "summary") == "summary":
        ...
        console.print(
            _t(
                f"Portfolio Overview · {target_curr.upper()}",
                [("Metric", {"style": "cyan"}), ("Value", {"style": "bold green"})],
                [
    ...
    for a in actions:
        val = (
            ...
            if a.action_type in (ActionType.SPLIT, ActionType.REVERSE_SPLIT)
                ...
                else (a.new_symbol or "-")
            )
        )
        rows.append([a.effective_date, a.action_type.value, val, a.description])
    console.print(
        _t(
            f"Corporate Actions for {symbol}",
            [
                ("Date", {"style": "dim"}),
                ("Type", {"style": "bold cyan"}),
                ("Ratio / Amount", {"justify": "right"}),
...
def cmd_features(args: argparse.Namespace) -> None:
    ...
    if (getattr(args, "action", "list") or "list") == "list":
        console.print(
            _t(
                "Registered ML & Quantitative Features",
                [
                    ("Feature Name", {"style": "bold cyan"}),
                    ("Description", {"style": "dim"}),
                ],
                [
                    [f["name"], f["description"]]
                    for f in FeatureStore().registry.list_all()
                ],
            )
        )
...
def cmd_schedule(args: argparse.Namespace) -> None:
    ...
    if action == "list":
        s.add_job(
            "EOD Snapshot", "@eod", lambda: "Snapshot completed", job_id="eod_snap"
        )
        s.add_job(
            "Hourly Rollup", "@hourly", lambda: "Rollup completed", job_id="hr_rollup"
        )
        console.print(
            _t(
                ...
                [
                    ("Job ID", {"style": "dim"}),
                    ("Name", {"style": "bold cyan"}),
                    ("Schedule", {"style": "yellow"}),
                    ("Next Run", {"style": "green"}),
                ],
                [
                    [
                        j.job_id,
                        j.name,
                        j.cron_expr,
                        time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(j.next_run)),
                    ]
...
def cmd_markets(args: argparse.Namespace) -> None:
    ...
    fx_parts = [
        f"USD/{k}: [bold cyan]{v:,.2f}[/bold cyan]"
        for k, v in fx_summary.items()
        if k != "USD"
    ]
...
def add_trading_parsers(sub) -> None:
    ...
    p_bt = sub.add_parser(
        "backtest",
        aliases=["bt"],
        help="Historical backtesting engine with point-in-time event replay",
    )
    p_bt.add_argument(
        "-s",
        "--strategy",
    ...
    p_risk = sub.add_parser(
        "risk",
        aliases=["var", "cvar"],
        help="Portfolio risk management, VaR, CVaR, and correlation analysis",
    )
    ...
    # Bar Database
    p_bars = sub.add_parser(
        "bars",
        aliases=["bardb", "candles"],
        help="Multi-timeframe OHLCV bar database and historical lookup",
    )
    p_bars.add_argument(
        "action",
        nargs="?",
        default="summary",
        choices=["summary", "query", "instruments"],
    )
    ...
    p_opt = sub.add_parser(
        "options",
        aliases=["opt", "greeks"],
        help="Options pricing models, Greeks chain, and implied volatility solver",
    )
    p_opt.add_argument(
        "action", nargs="?", default="price", choices=["price", "chain", "surface"]
    )
    ...
    p_news = sub.add_parser(
        "news",
        aliases=["sentiment"],
        help="Financial news aggregation, sentiment analysis, and entity extraction",
    )
    p_news.add_argument(
        "action",
        nargs="?",
        default="latest",
        choices=["latest", "analyze", "summary", "fetch"],
    )
    p_news.add_argument(
        "text",
        nargs="?",
        default="",
        help="Headline text to analyze (when action is analyze)",
    )
    p_news.add_argument(
        "-s",
        "--symbol",
        default=None,
        help="Stock or crypto ticker symbol (e.g. NVDA, AAPL, BTC)",
    )
    p_news.add_argument(
        "-l",
        "--limit",
        type=int,
        default=10,
        help="Maximum number of headlines to display",
    )
    p_news.add_argument(
        "--feed", default=None, help="Custom RSS feed URL or source identifier"
    )
    p_news.add_argument(
    ...
    p_alert = sub.add_parser(
        "alert",
        aliases=["alerts"],
        help="Real-time alert engine (price, spread, volume, drawdown)",
    )
    p_alert.add_argument(
        "action", nargs="?", default="list", choices=["list", "add", "summary"]
    )
    ...
    # Watchlist
    p_wl = sub.add_parser(
        "watchlist", aliases=["wl"], help="Named watchlist management"
    )
    ...
    p_port = sub.add_parser(
        "portfolio",
        aliases=["port"],
        help="Portfolio tracker with P&L attribution and benchmark comparison",
    )
    ...
    p_port.add_argument(
        "--currency",
        default="USD",
        help="Base reporting currency (e.g. USD, INR, EUR, JPY)",
    )
    ...
    p_ca = sub.add_parser(
        "corpact",
        aliases=["splits", "dividends"],
        help="Corporate actions processor: splits, dividends, ticker changes",
    )
    p_ca.add_argument(
        "action", nargs="?", default="list", choices=["list", "add", "adjust"]
    )
    ...
    p_feat = sub.add_parser(
        "features",
        aliases=["feat"],
        help="ML feature store: technical indicators and microstructure metrics",
    )
    ...
    p_sched = sub.add_parser(
        "schedule",
        aliases=["sched", "cron"],
        help="Scheduled tasks and automated EOD reports",
    )
    p_sched.add_argument("action", nargs="?", default="list", choices=["list", "eod"])
    p_sched.set_defaults(func=cmd_schedule)

    # Markets
    p_mkts = sub.add_parser(
        "markets",
        aliases=["venues", "world"],
        help="Global financial exchange directory, market clocks, and microstructure",
    )
