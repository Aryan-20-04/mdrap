...
class NewsFeed:
    ...
    def parse_rss_xml(self, xml_content: str, source: str = "RSS") -> list[NewsItem]:
        ...
        if "<!DOCTYPE" in upper_xml or "<!ENTITY" in upper_xml:
            raise ValueError(
                "XML entity expansion / DTD processing is forbidden for security"
            )
        try:
            ...
            for item in root.findall(".//item"):
                ...
                if title is not None and title.text:
                    ...
                    new_items.append(
                        self.add_headline(title.text, source=source, url=url)
                    )
    ...
    def sentiment_summary(self, symbol: str | None = None) -> dict[str, float | int]:
        ...
        bullish = sum(
            1
            for i in items
            if i.sentiment in {SentimentScore.BULLISH, SentimentScore.VERY_BULLISH}
        )
        bearish = sum(
            1
            for i in items
            if i.sentiment in {SentimentScore.BEARISH, SentimentScore.VERY_BEARISH}
        )
    ...
    def correlate_with_price(
        self, news_item: NewsItem, price_before: float, price_after: float
    ) -> dict[str, float | bool]:
        ...
        return {"price_change_pct": change_pct, "direction_match": direction_match}
