...
class PortfolioRiskEngine:
    def __init__(self, confidence_level: float = 0.95, lookback_window: int = 252, initial_capital: float = 100_000.0) -> None:
    ...
    def monte_carlo_var(self, n_simulations: int = 10000, horizon_days: int = 1, confidence: float | None = None, seed: int = 42) -> float:
        conf = confidence if confidence is not None else self.confidence_level
        returns = self._series.returns()[-self.lookback_window:]
        if len(returns) < 2:
            return 0.0
        ...
        if fastpath is not None:
            c_res = fastpath.fast_monte_carlo_var(mu, sigma, n_simulations, horizon_days, self._current_value, conf, seed)
    ...
    def sharpe_ratio(self, risk_free_rate: float = 0.0, annualization_factor: float = 252.0) -> float:
    ...
    def sortino_ratio(self, risk_free_rate: float = 0.0, annualization_factor: float = 252.0) -> float:
        returns = self._series.returns()
        if len(returns) < 2:
            return 0.0
        ...
        downside_variance = sum(r**2 for r in downside_returns) / (len(downside_returns) - 1) if len(downside_returns) > 1 else 0.0
...
class DrawdownCircuitBreaker:
    ...
    def __init__(self, warning_pct: float = 3.0, critical_pct: float = 5.0, kill_pct: float = 10.0) -> None:
